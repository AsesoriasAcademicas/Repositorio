<%-- 
    Document   : NuevoTrabajo
    Created on : 3/03/2018, 06:06:28 PM
    Author     : Alex
--%>
...
<%  
    ...
    if(sesion != null){
        ...
        habilitar = (String) sesion.getAttribute("habilitar");
    }
%>
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function personalizarElementos() {
                if (document.getElementById("dejanosTuTrabajo").firstChild.href == window.location.href) {
                    sessionStorage.clear();
                    document.getElementById("dejanosTuTrabajo").style.backgroundColor = "#FDFDFD";
                    document.getElementById("dejanosTuTrabajo").firstChild.style.color = "#1C242A";
                }
                ...
                if ('<%= session.getAttribute("confirmacionFallidaDejanosTuTrabajo")%>' != "" && '<%= session.getAttribute("confirmacionExitosaDejanosTuTrabajo")%>' == "") {
                    document.getElementById("confirmacionFallidaDejanosTuTrabajo").innerHTML = '<%= session.getAttribute("confirmacionFallidaDejanosTuTrabajo")%>';
                    document.getElementById("confirmacionFallidaDejanosTuTrabajo").style.display = "block";
                    document.getElementById("confirmacionExitosaDejanosTuTrabajo").style.display = "none";
                } else if ('<%= session.getAttribute("confirmacionFallidaDejanosTuTrabajo")%>' == "" && '<%= session.getAttribute("confirmacionExitosaDejanosTuTrabajo")%>' != "") {
                    document.getElementById("confirmacionExitosaDejanosTuTrabajo").innerHTML = '<%= session.getAttribute("confirmacionExitosaDejanosTuTrabajo")%>';
                    document.getElementById("confirmacionExitosaDejanosTuTrabajo").style.display = "block";
                    document.getElementById("confirmacionFallidaDejanosTuTrabajo").style.display = "none";
                }
                
                if('<%= session.getAttribute("habilitar")%>' == "true"){
           ...
           */
           ...
           La funcion deshabilitar() deshabilita los elementos del formulario formRegistro.

            */
            
            
            function deshabilitarFormulario(){
                contfrm = document.getElementsByClassName("contformulario");
                btnfrm = document.getElementsByClassName("buttonformulario");
                for(var i = 0; i < contfrm.length; i++){
                    contfrm[i].removeAttribute("disabled");
                }
                for(var i = 0; i < btnfrm.length; i++){
                    btnfrm[i].removeAttribute("disabled");
                }
            }
    ...
    <body onload="personalizarElementos()">
        ...
        <section id="contenido">
            ...
            <article>
                <form id="formUpload" action="subirArchivo" method="POST" enctype="multipart/form-data" >
                    <article>
                       <section class="datoformulario">
                            <input type="file" name="archivo" id="archivo" accept="image/jpeg,image/gif,image/png,application/pdf,.txt,.doc,.docx" required/>
                        </section>
                ...
                <form id="formRegistro" action="trabajo" method="POST" onsubmit="return validar()">
                    <fieldset id="datosHorario">
                        ...
                        <article>
                            <section class="datoformulario">
                                ...
                                <input type="date" name="fecha" id="fecha" class="contformulario" disabled="true" required>
                            </section>
                            <section class="datoformulario">
                                <label for="hora" class="obligatorio">Hora entrega</label>
                                <label id="validhora"></label>
                                <input type="time" name="hora" min="07:00" max="18:00" id="hora" class="contformulario" disabled="true" required>
                            </section>
                            <!--<section class="confirmarformulario">
                                <button type="button" id="btnSiguiente" class="buttonformulario" onclick="siguienteStep('datosHorario')" disabled="true">Siguiente</button>
                            </section>-->
                        </article>
                    </fieldset>
                    ...
                    <fieldset id="datosClase">
                        ...
                        <article>
                            <section class="datoformulario">
                                ...
                                <select id="primary" name="materia" id="materia" class="contformulario" disabled="true" autofocus>
                            ...
                            <section class="datoformulario">
                                ...
                                <select id="secondary" name="tema" id="tema" class="contformulario" disabled="true">
                                </select>
                            </section>
                            <section class="datoformulario">
                                <label for="mensaje" class="obligatorio">Dudas/inquietudes</label>
                                <label id="validmensaje"></label>
                                <textarea name="mensaje" id="mensaje" placeholder="Escriba aquí sus dudas e inquietudes..." class="contformulario" maxlength="500" <%if (session.getAttribute("archivo")!= null) {%> value="<%= session.getAttribute("archivo")%>" <%}%> disabled="true" required></textarea>
                            </section>
                            <!--<section class="confirmarformulario">
                                <button type="button" id="btnAnterior" class="buttonformulario" onclick="anteriorStep('datosClase')" disabled="true">Anterior</button>
                                <button type="button" id="btnSiguiente" class="buttonformulario" onclick="siguienteStep('datosClase')" disabled="true">Siguiente</button>
                            </section>-->
                        </article>
                    </fieldset>
                    ...
                    <fieldset id="datosEstudiante">
                        ...
                        <article>
                            <section class="datoformulario">
                                ...
                                <input type="text" name="nombre" id="nombre" class="contformulario" pattern="^([a-zA-Z\s])*$" maxlength="50" <%if(usu != null){%> value="<%= usu.getNombrePersona()%>" <%}%> disabled="true" required>
                            </section>
                            <section class="datoformulario">
                                <label for="direccion" class="obligatorio">Dirección</label>
                                <label id="validdireccion"></label>
                                <input type="text" name="direccion" id="direccion" class="contformulario" <%if(usu != null){%> value="<%= usu.getDireccionPersona()%>" <%}%> disabled="true" required>
                            </section>
                            <section class="datoformulario">
                                <label for="barrio" class="obligatorio">Barrio</label>
                                <label id="validbarrio"></label>
                                <input type="text" name="barrio" id="barrio" class="contformulario" <%if(usu != null){%> value="<%= usu.getBarrioResidenciaPersona()%>" <%}%> disabled="true" required>
                            </section>
                            <section class="datoformulario">
                                <label for="telefono" class="obligatorio">Teléfono</label>
                                <label id="validtelefono"></label>
                                <input type="tel" name="telefono" id="telefono" class="contformulario" pattern="^[0-9]{7}|[0-9]{10}$" placeholder="Fijo 8****** - Móvil 3*********" <%if(usu != null){%> value="<%= usu.getTelefonoPersona()%>" <%}%> disabled="true" required>
                            </section>
                            <section class="confirmarformulario">
                                <!--<button type="button" id="btnAnterior" class="buttonformulario" onclick="anteriorStep('datosEstudiante')" disabled="true">Anterior</button>-->
                                <input type="submit" id="btnEnviar" class="buttonformulario" value="Enviar" onclick="return confirm('¿Está seguro(a) de guardar el nuevo Trabajo?');" disabled="true">
                            </section>
                        </article>
                    </fieldset>
                </form>
