<%-- 
    Document   : EditarClasePresencial
    Created on : 5/03/2018, 01:10:24 PM
    Author     : Alex
--%>
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function personalizarElementos() {
                <% session.setAttribute("codTut", request.getParameter("CodigoTutoria")); %>
                ...
                document.getElementById("fecha").value = "<%= cls.getFechaClase()%>"; 
                document.getElementById("hora").value = "<%= cls.getHoraClase()%>";
                //document.getElementById("datosEstudiante").style.display = "none";
                //document.getElementById("datosClase").style.display = "none";
                
                if ('<%= session.getAttribute("confirmacionFallidaEditaClasePresencial")%>' != "" && '<%= session.getAttribute("confirmacionExitosaEditaClasePresencial")%>' == "") {
                    document.getElementById("confirmacionFallidaEditaClasePresencial").innerHTML = '<%= session.getAttribute("confirmacionFallidaEditaClasePresencial")%>';
                    document.getElementById("confirmacionFallidaEditaClasePresencial").style.display = "block";
                    document.getElementById("confirmacionExitosaEditaClasePresencial").style.display = "none";
                } else if ('<%= session.getAttribute("confirmacionFallidaEditaClasePresencial")%>' == "" && '<%= session.getAttribute("confirmacionExitosaEditaClasePresencial")%>' != "") {
                    document.getElementById("confirmacionExitosaEditaClasePresencial").innerHTML = '<%= session.getAttribute("confirmacionExitosaEditaClasePresencial")%>';
                    document.getElementById("confirmacionExitosaEditaClasePresencial").style.display = "block";
                    document.getElementById("confirmacionFallidaEditaClasePresencial").style.display = "none";
                }
            }
                    ...
                    case 'datosHorario':
                        ...
                        for (var i = 0; i < campos.length; i++) {
                            if (!campos[i].checkValidity()) {
                                /*
                                 * Permite editar el mensaje predeterminado de un input
                                 * 
                                 */

                                //document.getElementById("duracion").setCustomValidity("This email is already used");
                                return false;
                            }
                        }
                        break;
            ...
            function confirmacionSalirVista() {
                ...
                obtenerValorParametro('CodigoTutoria');
            }
            ...
            $(function () {
                document.getElementById("primary").value = "<%= cls.getTutoria().getAsignaturaTutoria()%>";
                ...
                document.getElementById("secondary").value = "<%= cls.getTutoria().getTemaTutoria()%>";
            })
    ...
    <body onload="personalizarElementos()">
        ...
        <section id="contenido">
            ...
            <article>
                <p id="confirmacionExitosaEditaClasePresencial" class="confirmacionExitosa"></p>
                <p id="confirmacionFallidaEditaClasePresencial" class="confirmacionFallida"></p>
                ...
                <form id="formRegistro" action="modificarClasePresencial" method="POST" onsubmit="return confirm('¿Está seguro(a) de guardar los cambios de la Clase Presencial?');">
                    <fieldset id="datosHorario">
                        ...
                        <article>
                            <section class="datoformulario">
                                ...
                                <input type="date" name="fecha" id="fecha" class="contformularioDatosHorario" <%if (cls != null) {%> value="<%= cls.getFechaClase()%>" <%}%> required>
                            </section>
                            <section class="datoformulario">
                                <label for="hora" class="obligatorio">Hora</label>
                                <label id="validhora"></label>
                                <input type="time" name="hora" id="hora" min="07:00" max="18:00" class="contformularioDatosHorario" <%if (cls != null) {%> value="<%= cls.getHoraClase()%>" <%}%> required>
                            </section>
                            <section class="datoformulario">
                                <label for="duracion" class="obligatorio">Duración(Horas)</label>
                                <label id="validduracion"></label>
                                <input type="number" name="duracion" min="1" max="6" id="duracion" class="contformularioDatosHorario" <%if (cls != null) {%> value="<%= cls.getDuracionClase()%>" <%}%> required>
                            </section>                            
                    ...
                    <fieldset id="datosClase">
                        ...
                        <article>
                            <section class="datoformulario">
                                ...
                                <select id="primary" name="materia" class="contformulario" autofocus>
                            ...
                            <section class="datoformulario">
                                ...
                                <select id="secondary" name="tema" class="contformulario">
                                </select>
                            </section>
                            <section class="datoformulario">
                                <label for="mensaje" class="obligatorio">Dudas/inquietudes</label>
                                <label id="validmensaje"></label>
                                <textarea name="mensaje" id="mensaje" placeholder="Escriba aquí sus dudas e inquietudes..." class="contformularioDatosClase"  maxlength="500" required><%= cls.getTutoria().getDudasInquietudesTutoria()%></textarea>
                            </section>
                            <!--<section class="confirmarformulario">
